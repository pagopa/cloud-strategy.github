...
3. Create only the narrowest local asset set that solves the request.
4. Keep repository-owned prompt, skill, and agent filenames plus frontmatter `name:` values on the `local-*` convention.
5. Update the target `AGENTS.md` inventory and routing only as needed, keeping `.github/...` paths explicit.
6. Do not create local duplicates of existing `tech-ai-*` capabilities unless the repo-specific behavior materially differs.
7. Report whether the capability should remain local or be proposed for promotion into the shared source baseline.
