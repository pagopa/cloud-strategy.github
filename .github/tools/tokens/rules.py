...
from common.paths import is_imported_asset, iter_markdown_assets
...
def check_root_policy_overlap(root: Path) -> list[Finding]:
    ...
    shared_lines = significant_text_lines(
        read_text(agents_path)
    ) & significant_text_lines(read_text(copilot_path))
...
def check_imported_skill_description_budget(root: Path) -> list[Finding]:
    ...
    for path in iter_skill_paths(root):
        ...
        if (
            not is_imported_asset(relative_path)
            or relative_path in profiled_skill_paths
        ):
            continue
...
def check_skill_description_trigger_collisions(root: Path) -> list[Finding]:
    ...
    for path in iter_skill_paths(root):
        ...
        if len(normalized_description) < 40:
            continue
        paths_by_description[normalized_description].append(
...
def check_internal_root_policy_overlap(root: Path) -> list[Finding]:
    ...
    root_policy_lines = significant_text_lines(
        read_text(agents_path)
    ) | significant_text_lines(read_text(copilot_path))
    findings: list[Finding] = []
...
def check_paired_agent_skill_overlap(root: Path) -> list[Finding]:
    ...
    for agent_path in iter_repo_owned_agent_paths(root):
        ...
        mandatory_skills = [
            bullet.strip("`")
            for bullet in extract_section_bullets(
                agent_text, "## Mandatory Engine Skills"
            )
        ]
        ...
        overlap = {
            line
            for line in significant_text_lines(agent_text)
            & significant_text_lines(read_text(skill_path))
            if len(line) >= 24
        }
        if len(overlap) < 6:
            continue
