...
def build_manifest(
    root: Path, files: list[Path], source_ref_base: str | None = None
) -> dict[str, object]:
    return {
...
def build_fingerprint(
    root: Path, file_path: Path, source_ref_base: str | None = None
) -> ResourceFingerprint:
...
def index_resources(manifest: dict[str, object]) -> dict[str, dict[str, object]]:
    return {
        resource["resource_id"]: resource for resource in manifest.get("resources", [])
    }
