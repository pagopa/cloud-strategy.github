...
- Detect catalog retirements or remaps that were not propagated in the same change to the host catalog consistency check, when the host exposes one.
...
An audit is complete only when every catalog area in scope has been audited and each finding is recorded with its asset path. If the host exposes a catalog consistency check, run it; if it does not, record that the check is unavailable and still complete the audit.
