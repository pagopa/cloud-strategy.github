...
- `.github/local-github-instructions-overrides.md`: consumer-local exception layer authorized by `AGENTS.md`; it may override synced defaults only when conflict, scope, reason, and disclosure are explicit
- `.github/INVENTORY.md`: exact live catalog generated from target filesystem state

Do not flatten these roles into one file. Do not let target `AGENTS.md` become an inventory dump or a second full copy of `.github/copilot-instructions.md`. Do not let `.github/local-github-instructions-overrides.md` replace the bridge or catalog roles.
...
- preserved `local-*` assets and `.github/local-github-instructions-overrides.md` status
