...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `codex` | Skills | Documented | `$HOME/.agents/skills/<skill>/` | Yes | No | Yes | Direct-copy bundles with `SKILL.md`, `references/`, `scripts/`, `assets/`, and `agents/openai.yaml`. |
...
| `vscode` | Compatibility aliases | Documented | `~/.claude/skills/<skill>/`, `~/.agents/skills/<skill>/` | Yes | No | No | Support only when the user explicitly selects the alias path. |
...
| `antigravity` | Skills | Unknown / To verify | `~/.gemini/antigravity/skills/<skill>/` | Possibly | No if documented | No | Keep available only for explicit experimental runs. |
...
| `antigravity` | Project-local skills | Unknown / To verify | `.agents/skills/` or equivalent | No | No | No | Workspace-local, not home sync. |
| `antigravity` | Project-local rules | Unknown / To verify | `.agents/rules/` or equivalent | No | Unknown | No | Workspace-local, not home sync. |
...
- Treat `Unknown / To verify` rows as `plan`, `audit`, and `doctor` only unless the user explicitly enables an experimental run.
