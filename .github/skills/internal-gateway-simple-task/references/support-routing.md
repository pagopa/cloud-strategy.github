...
| Executable behavior change | Load `/internal-tdd` to classify `mandatory-test-first`, `feature-first`, `prototype-unverified`, or `validation-only` routing when a meaningful executable or evaluable seam exists. | Do not force it onto pure prose or governance wording with no executable seam. |
