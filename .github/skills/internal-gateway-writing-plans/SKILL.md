...
## Normative Manifest v3 Contract
...
A current Manifest v3 has exactly these top-level fields, with no additions or
omissions, and its identity is exactly `schema_version: 3` with
...
There is no legacy manifest parser compatibility mode. Imported or legacy plans
must be regenerated by the writer route as Manifest v3, then approved and
preflighted.
...
- Current `delegation` has exactly `schema_version`, `mode`, `worker`,
  ...
  Manifest v3 does not manufacture delegated worker provenance.
...
- `rollout` is a non-empty list of strings. `handoff` has exactly
  ...
  `/internal-gateway-execute-plans`, its requirements include human approval,
   exact Manifest v3 review, and zero blocking preflight findings, and
