...
- The local Graphify output under `tmp/graphify/graphify-out/` already exists, or the task explicitly allows refreshing it with `make graphify-update`.
...
- Canonical output path: `tmp/graphify/graphify-out/graph.json`
- Working corpus path: `tmp/graphify/`
...
1. Check whether `tmp/graphify/graphify-out/graph.json` exists and is fresh enough for the current question.
