...
- If `internal-router` already routed the request straight to `internal-kubernetes-deployment` or `antigravity-kubernetes-architect`, treat that direct route as the audited winner and do not insert `internal-kubernetes` as an extra selector layer.
