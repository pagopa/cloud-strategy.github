...
| Workload manifests under `k8s/`, `manifests/`, `deploy/`, or Helm templates | `internal-kubernetes-deployment` | The work is about controller choice, rollout behavior, probes, exposure, config, or hardening | |
...
| Kubernetes delivery that also changes GitHub Actions workflows | `internal-kubernetes-deployment` plus `/internal-github` | One skill owns the workload delivery shape and the other routes CI/CD workflow changes | Keep workflow authoring out of the Kubernetes skill unless the YAML pipeline is the actual target |
