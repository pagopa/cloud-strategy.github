...
| Workload manifests under `k8s/`, `manifests/`, `deploy/`, or Helm templates | `internal-kubernetes-deployment` | The work is about controller choice, rollout behavior, probes, exposure, config, or hardening | Let `internal-kubernetes-manifests.instructions.md` auto-apply for YAML hygiene |
