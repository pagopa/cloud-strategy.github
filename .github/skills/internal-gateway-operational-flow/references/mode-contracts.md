...
- `define` exits to `plan`, `review`, or critical challenge only through a
  ...
  explicitly requested `define-first` only. For `define-first`, brainstorming,
  and clarify-first entrypoints, closing Gate 0 does not change the active
  phase; agreement, option selection, accepted defaults, or approval-like
  replies update the definition but do not authorize plan output.
