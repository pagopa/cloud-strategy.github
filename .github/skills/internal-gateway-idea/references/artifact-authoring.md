...
`internal-luna-executor` agent through `/internal-subagent-contract` with one
bounded `DelegationBrief`. The active primary owner may author locally only
when the active authoring model is already GPT-5.6 Luna.
...
review, validation, and handoff. That owner must delegate plan authoring to the
`internal-luna-executor` agent through `/internal-subagent-contract` with one
bounded `DelegationBrief`. The active primary owner may author locally only
when the active authoring model is already GPT-5.6 Luna.
...
Luna authors the selected artifact; the caller remains responsible for
eligibility, routing, authority, result verification, acceptance, and closeout.
