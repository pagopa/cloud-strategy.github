...
Every authored README resolves the diagram question inside the document, and the two outcomes are exhaustive: it carries a diagram, or it carries one sentence opening with `No diagram is provided` that states why this target stays below the threshold, reported as `omitted-with-reason`. Where another document already draws the relationship, the omission sentence links to it; never redraw a diagram a second document owns. The sentence is fixed so that the disposition of an entire repository is auditable with a plain text search and no validator. A README carrying neither outcome is incomplete, not undecided.
