...
Use a diagram only when it clarifies evidenced relationships for the reader. Preserve unresolved evidence as `Unknown / To verify` rather than filling a section with invented content. Preserve any existing generated block byte-for-byte and keep authored architecture prose before its owner marker.
