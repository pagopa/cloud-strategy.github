...
from .jsonc import JsoncParseError, apply_managed_vscode_copilot_settings
from .shared import (
    ARCHITECTURE_PATH,
...
@dataclass(frozen=True)
class ConsumerLocalKnowledgeSpec:
    path: str
    preserve_reason: str
    create_reason: str
...
CONSUMER_LOCAL_KNOWLEDGE_SPECS = (
    ConsumerLocalKnowledgeSpec(
        path=DOCS_README_PATH,
        preserve_reason="Preserved consumer-local docs guide after scaffold materialization.",
        create_reason="Consumer-local docs guide missing; create scaffold from the source template.",
    ),
    ConsumerLocalKnowledgeSpec(
        path=ARCHITECTURE_PATH,
        preserve_reason="Preserved consumer-local architecture contract after scaffold materialization.",
        create_reason="Consumer-local architecture contract missing; create scaffold from the source template.",
        legacy_paths=ARCHITECTURE_LEGACY_PATHS,
    ),
    ConsumerLocalKnowledgeSpec(
        path=REPOSITORY_CONTEXT_PATH,
        preserve_reason="Preserved consumer-local repository context after scaffold materialization.",
        create_reason="Consumer-local repository context missing; create scaffold from the source template.",
        legacy_paths=REPOSITORY_CONTEXT_LEGACY_PATHS,
    ),
    ConsumerLocalKnowledgeSpec(
        path=TECH_PATH,
        preserve_reason="Preserved consumer-local technology document after scaffold materialization.",
        create_reason="Consumer-local technology document missing; create scaffold from the source template.",
    ),
    ConsumerLocalKnowledgeSpec(
        path=STRUCTURE_PATH,
        preserve_reason="Preserved consumer-local structure document after scaffold materialization.",
        create_reason="Consumer-local structure document missing; create scaffold from the source template.",
    ),
)
...
def append_consumer_local_knowledge_operations(
    ...
    for spec in CONSUMER_LOCAL_KNOWLEDGE_SPECS:
        _append_consumer_knowledge_operation(
            source_root=source_root,
            target_root=target_root,
            operations=operations,
            local_assets=local_assets,
...
def _append_consumer_knowledge_operation(
    ...
    operations: list[SyncOperation],
    local_assets: list[str],
    spec: ConsumerLocalKnowledgeSpec,
) -> None:
    template_path = source_root / CONSUMER_LOCAL_KNOWLEDGE_TEMPLATES[spec.path]
    target_path = target_root / spec.path
    source_hash = sha256_file(template_path) if template_path.exists() else None
    legacy_paths = [
        legacy_path
    ...
    if target_path.exists() and legacy_paths:
        local_assets.append(spec.path)
        operations.append(
            SyncOperation(
                action="manual",
                path=spec.path,
                reason=(
                    f"Both canonical {spec.path} and legacy {', '.join(legacy_paths)} exist; reconcile manually before apply."
                ),
                source_hash=source_hash,
                target_hash=sha256_file(target_path),
            )
    ...
    if len(legacy_paths) > 1:
        operations.append(
            SyncOperation(
                ...
                path=spec.path,
                reason=(
                    f"Multiple legacy paths for {spec.path} exist ({', '.join(legacy_paths)}); reconcile manually before apply."
                ),
                source_hash=source_hash,
    ...
    if legacy_paths:
        ...
        operations.append(
            SyncOperation(
                action="rename",
                path=spec.path,
                reason=f"Legacy {legacy_path} should move to consumer-local {spec.path}.",
                source_hash=None,
