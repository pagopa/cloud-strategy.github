...
name: TechAICustomizationAuditor
...
## Status
Deprecated compatibility alias for `TechAIStandardsRepoConfigAuditor`. Use the global auditor for new customization work in this repository.
...
- If findings exist, route `Critical` and `Major` findings back to `TechAIImplementer` with concrete remediation steps.
