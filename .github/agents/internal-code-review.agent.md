...
- Treat preferred or optional skills as a three-lane review toolkit: use `obra-*` for review framing, response handling, investigation, stronger regression discipline, and evidence; use `internal-*` as the tactical defect-first owner; use imported skills only for narrow security-analysis support.
...
- `obra-systematic-debugging`: Use when the failure mechanism is unclear and the review needs a root-cause-first investigation path before concluding.
- `obra-test-driven-development`: Use when review findings expose missing failing tests, brittle regression coverage, or weak behavioral proof around the change.
