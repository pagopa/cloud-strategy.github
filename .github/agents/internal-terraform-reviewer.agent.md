...
name: internal-terraform-reviewer
...
- Load matching cloud-specific instruction file when available:
  - AWS → `.github/instructions/internal-terraform-aws.instructions.md`
  - Azure → `.github/instructions/internal-terraform-azure.instructions.md`
  - GCP → `.github/instructions/internal-terraform-gcp.instructions.md`
...
Load and apply `.github/skills/internal-code-review/SKILL.md` Terraform section as the primary anti-pattern catalog. Cross-reference with `.github/instructions/internal-terraform.instructions.md`.

## Escalation rules

- Any single anti-pattern repeated 3+ times in the same diff escalates one severity level.
- Any deviation from `internal-terraform.instructions.md` or cloud-specific instruction file is at minimum a `Nit`.
...
- If the review surfaces IAM/privilege concerns beyond Terraform, suggest `internal-security-reviewer`.
