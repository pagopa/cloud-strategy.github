...
- For a generic `sync` request, follow the default sync sequence from the paired skill: run install `plan` first for the default `skills` target, stop on blockers, then run `bisync plan`.
