...
- Repo-only consumer-local specialist: `TechAILocalCopilotCustomizationBuilder`.

## Repo-only agents (not synced to consumers)
- `TechAIGlobalCustomizationBuilder`
- `TechAIGlobalCustomizationAuditor`
- `TechAILocalCopilotCustomizationBuilder`
...
10. Use `TechAISyncCopilotConfigs` to align a consumer baseline before creating repo-owned local assets.
11. Use `TechAILocalCopilotCustomizationBuilder` for repo-owned `local-*` prompts, skills, agents, and `AGENTS.md` updates that should stay consumer-local.
