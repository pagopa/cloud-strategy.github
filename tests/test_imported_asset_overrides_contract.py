...
def test_imported_asset_override_registry_hashes_match_live_targets() -> None:
    ...
    for entry in payload["overrides"]:
        ...
        assert build_fingerprint(repo_root, target_path).content_hash == entry["expected_content_hash"]


def test_imported_asset_override_policy_is_visible_in_canonical_and_sync_assets() -> None:
    agents_text = Path("AGENTS.md").read_text(encoding="utf-8")
    copilot_text = Path(".github/copilot-instructions.md").read_text(encoding="utf-8")
    sync_agent_text = Path(".github/agents/internal-sync-control-center.agent.md").read_text(
        encoding="utf-8"
    )
