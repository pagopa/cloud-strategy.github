...
SCRIPT_TESTS_ROOT = REPO_ROOT / ".github/scripts/tests"
...
def test_github_owned_python_tests_make_owner_obvious() -> None:
    ...
    for test_root in (TESTS_ROOT, SCRIPT_TESTS_ROOT):
        for test_path in sorted(test_root.rglob("test_*.py")):
            ...
            repo_rel_path = test_path.relative_to(REPO_ROOT).as_posix()
...
def test_pytest_config_discovers_root_and_skill_test_roots() -> None:
    ...
    assert SCRIPT_TESTS_ROOT.is_dir(), "scripts test root must exist"
