...
def test_valid_creator_result_binds_artifact_acceptance_and_progress() -> None:
    brief_path = FIXTURES / "valid-plan-brief.json"
    brief = _load(brief_path.name)
...
def test_creator_result_rejects_tampered_binding(tamper: str) -> None:
    brief_path = FIXTURES / "valid-plan-brief.json"
    brief = _load(brief_path.name)
    result = copy.deepcopy(_load("valid-plan-result.json"))
