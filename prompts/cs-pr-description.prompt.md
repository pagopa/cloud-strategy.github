...
4. Do not remove required template sections; fill not-applicable content with `N/A`.
5. In `Changes`, provide brief bullets aligned to `changed_files`.
6. In `Scope`, check only applicable categories.
7. Keep content concise, concrete, and in English.
8. If PR tools are available:
   - update existing PR when found
   - otherwise create draft PR and then update title/body
   - verify persisted body includes all required headings
9. If PR tools are unavailable, return ready-to-paste markdown plus exact CLI fallback commands.
