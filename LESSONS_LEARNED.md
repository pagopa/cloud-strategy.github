...
| 2026-04-29 | When promoting `.github/prompts/` to source-managed catalog assets, update inventory generation and sync discovery in the same pass; inventory-only changes leave consumer sync blind to prompt assets. | pending | `AGENTS.md` |
...
| 2026-04-29 | When introducing exact `applyTo` overlap validation, run it across the whole repository immediately; the first intentional exception can expose other baseline-plus-extension co-loads that also need explicit allowlist coverage. | pending | `.github/skills/internal-copilot-audit/SKILL.md` |
...
| 2026-04-29 | Token-risk checks for skills should rely on observable repository signals such as description length, exact trigger collisions, and repo-profile coverage; do not infer runtime token cost from assumed skill-loading behavior. | pending | `.github/skills/internal-copilot-audit/SKILL.md` |
