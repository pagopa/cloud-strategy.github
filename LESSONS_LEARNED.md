...
| 2026-04-23 | In GitHub Actions, do not place `runner.temp`-derived paths in workflow-level or `jobs.<job_id>.env`; initialize those paths in a step that writes to `GITHUB_ENV`, or resolve them directly inside `run`, because `runner` is not available in those `env` scopes. | pending | Repository GitHub Actions workflow conventions |
| 2026-04-23 | When a CI smoke test exercises a repository wrapper around an external CLI, install that CLI on the runner even for `--dry-run` paths if the wrapper validates dependencies before dispatch; dry-run does not bypass preflight binary checks. | pending | Repository CI smoke-test conventions |
