...
| Catalog generation | `.github/tools/inventory/build-inventory.py` and `./.github/tools/run.sh build-inventory` |
| Contract checks | `pytest` tests under `tests/` and `.github/skills/*/tests/`, plus validator subcommands through `.github/tools/run.sh` |
...
- Skill-owned tests live under the owning `.github/skills/<skill>/tests/` bundle;
  root `tests/` mirrors repository automation and owns cross-boundary contracts.
